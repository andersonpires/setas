<?php
/**
 * Importação: vale_gas_federal
 * Origem: !Suporte/base_dados/vale_gas_federal.CSV
 * Colunas: id_vale_gas_federal (auto), cpf, nis, nome, situacao (SITBENEFICIO), id_beneficiario (em branco)
 * CPF: zeros ao final até 11 caracteres
 * NIS: NULL se vazio
 *
 * Execute: php import_vale_gas_federal.php
 * Ou acesse: http://localhost/setas-web/database/import_vale_gas_federal.php
 */
...
$csvPath = BASE_PATH . '/!Suporte/base_dados/vale_gas_federal.CSV';

echo "<pre style='font-family:monospace;'>";
echo "=== Importação vale_gas_federal ===\n\n";
...
try {
    ...
    $pdo->exec("DROP TABLE IF EXISTS `vale_gas_federal`");
    $pdo->exec("SET FOREIGN_KEY_CHECKS = 1");

    $pdo->exec("
        CREATE TABLE `vale_gas_federal` (
    ...
    ");
    echo "[OK] Tabela vale_gas_federal criada com nova estrutura.\n\n";
    ...
    $idxSituacao = obterColuna($header, ['SITBENEFICIO', 'situacao']);
    ...
    for ($i = 0; $i < count($registros); $i += $batchSize) {
        ...
        $pdo->prepare("INSERT INTO vale_gas_federal (cpf, nis, nome, situacao, id_beneficiario) VALUES " . $placeholders)->execute($params);
    }

    $total = (int) $pdo->query("SELECT COUNT(*) FROM vale_gas_federal")->fetchColumn();
