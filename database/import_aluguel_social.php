<?php
/**
 * Importação: aluguel_social (Aluguel Social)
 * Origem: !Suporte/base_dados/aluguel_social.CSV
 * Colunas: id_aluguel_social (auto), cpf, nis, nome, id_beneficiario (em branco)
 * CPF: remove "." e "-", apenas números (11 caracteres)
 *
 * Execute: php import_aluguel_social.php
 * Ou acesse: http://localhost/setas-web/database/import_aluguel_social.php
 */
...
$csvPath = $basePath . 'aluguel_social.CSV';

echo "<pre style='font-family:monospace;'>";
echo "=== Importação aluguel_social ===\n\n";
...
try {
    ...
    $pdo->exec("DROP TABLE IF EXISTS `aluguel_social`");
    $pdo->exec("SET FOREIGN_KEY_CHECKS = 1");

    $pdo->exec("
        CREATE TABLE `aluguel_social` (
    ...
    ");
    echo "[OK] Tabela aluguel_social criada com nova estrutura.\n\n";
    ...
    for ($i = 0; $i < count($registros); $i += $batchSize) {
        ...
        $pdo->prepare("INSERT INTO aluguel_social (cpf, nis, nome, id_beneficiario) VALUES " . $placeholders)->execute($params);
    }

    $total = (int) $pdo->query("SELECT COUNT(*) FROM aluguel_social")->fetchColumn();
